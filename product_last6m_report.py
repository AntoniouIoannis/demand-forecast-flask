...
def main():
    ...
    ap.add_argument(
        "--last_n_months", type=int, default=6, help="How many last months to include"
    )
    args = ap.parse_args()
    ...
    if len(months) < args.last_n_months:
        raise ValueError(
            f"predictions only has {len(months)} months; cannot take last {args.last_n_months}"
        )
    ...
    report = report.sort_values(
        ["actual_total", "wape"], ascending=[False, True]
    ...
    pivot_true = pred_last.pivot_table(
        index="product_id",
        columns="month",
        values="y_true",
        aggfunc="sum",
    ...
    pivot_pred = pred_last.pivot_table(
        index="product_id",
        columns="month",
        values="y_pred",
        aggfunc="sum",
    ...
    pivot_true.columns = [
        f"y_true_{pd.Timestamp(c).strftime('%Y-%m')}" for c in pivot_true.columns
    ]
    pivot_pred.columns = [
        f"y_pred_{pd.Timestamp(c).strftime('%Y-%m')}" for c in pivot_pred.columns
    ]
    ...
    print(f" - {out_wide}")
    print(
        f"Months included: {[pd.Timestamp(m).strftime('%Y-%m') for m in last_months]}"
    )
